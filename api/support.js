...
function paypalDonateConfig() {
  const environment = process.env.PAYPAL_DONATE_ENVIRONMENT === 'sandbox' ? 'sandbox' : 'production';
  return {
    environment,
    hostedButtonId: process.env.PAYPAL_DONATE_HOSTED_BUTTON_ID || '',
  };
}
...
export default async function handler(req, res) {
  ...
  if (action === 'paypal-config') {
    ...
    const config = paypalDonateConfig();
    return sendJson(res, { configured: Boolean(config.hostedButtonId), ...config });
  }
