...
export function buildComplianceMatrix() {
  return COMPLIANCE_REQUIREMENTS.map((requirement) => ({
    ...requirement,
    evidence: requirement.evidence.map((item) => ({
      ...item,
      document: findDocumentByCode(item.document_code),
    })),
  }));
}

export function evaluateRequirement(requirementId) {
  const requirement = COMPLIANCE_REQUIREMENTS.find((entry) => entry.id === requirementId);
  if (!requirement) {
    return { id: requirementId, status: 'Not Applicable', summary: 'Requirement not found in registry.', evidence: [] };
  }
  return {
    id: requirement.id,
