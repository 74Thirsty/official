/**
 * @file        legacy.js
 * @description ACE legacy compatibility layer
...
function convertLegacyWorkflow(workflow) {
  ...
  const lifecycle = (workflow.requirements || []).map((requirement) => ({
    ...
    when: requirement.when || null,
    blockingRules: requirement.blockingRules || [],
  }));
  return {
    id: workflow.id,
    name: workflow.intent,
    ...
    workflowVersion: workflow.workflowVersion || 'legacy-1.0',
    legacy: true,
