/**
 * @file        engine.js
 * @description ACE engine — workflow execution, gate enforcement, record management
...
import { CANONICAL_POSITIONS } from '../onboarding-references.generated.js';
...
function fieldApplies(field, sectionId, allValues, pendingValues = {}) {
  if (!field.when) return true;
  const sourceSection = field.when.sectionId || sectionId;
  const source = sourceSection === sectionId
    ? { ...(allValues?.[sourceSection] || {}), ...pendingValues }
    : (allValues?.[sourceSection] || {});
  const actual = source[field.when.field];
  if (Array.isArray(field.when.in)) return field.when.in.includes(actual);
  if (Object.hasOwn(field.when, 'equals')) return actual === field.when.equals;
  return true;
}

function conditionMatches(instance, condition) {
  if (!condition) return true;
  if (Array.isArray(condition.all)) return condition.all.every((item) => conditionMatches(instance, item));
  if (Array.isArray(condition.any)) return condition.any.some((item) => conditionMatches(instance, item));
  const actual = instance.fieldValues?.[condition.sectionId]?.[condition.field];
...
export function stageApplies(instance, stage) {
  return conditionMatches(instance, stage?.when);
}

export function validateFieldValues(template, sectionId, payload, allValues = {}) {
  ...
  const errors = [];
  const values = {};
  const definedFields = new Set((section.fields || []).map((field) => field.name));
  const prohibitedRestrictedInput = Object.keys(payload || {}).find((name) => !definedFields.has(name) && /^(?:ssn|social_security(?:_number)?|date_of_birth|dob|w4(?:_contents)?|i9(?:_contents)?|government_id(?:_number)?|identity_document|bank_(?:account|routing)(?:_number)?)$/i.test(name));
  if (prohibitedRestrictedInput) {
    return { errors: [`${prohibitedRestrictedInput} is restricted information and cannot be submitted to this compliance-record endpoint. Complete it in the approved restricted HR/payroll system and record only its status and opaque reference.`], values: {} };
  }
  for (const field of section.fields || []) {
    if (!fieldApplies(field, sectionId, allValues, payload)) continue;
    if (field.type === 'derived') {
      values[field.name] = field.derivedValue;
      continue;
    }
    if (field.type === 'derived-reference') {
      const selected = payload[field.deriveFrom] ?? allValues?.[sectionId]?.[field.deriveFrom];
      const reference = (field.references || []).find((item) => item.value === selected);
      if (field.required && !reference) errors.push(`${field.label} cannot be resolved from the canonical selection.`);
      else if (reference) values[field.name] = reference.description;
      continue;
    }
    ...
    if (field.type === 'select' && value && optionValues.length && !optionValues.includes(value)) {
...
export function sectionIsSatisfied(instance, workflow, sectionId) {
  ...
  for (const field of section.fields || []) {
    if (!fieldApplies(field, sectionId, instance.fieldValues)) continue;
...
function sectionMissingFields(instance, workflow, sectionId) {
  const template = instanceTemplate(instance, workflow);
  const section = (template?.sections || []).find((item) => item.id === sectionId);
  if (!section) return { sectionName: sectionId, fields: ['section definition unavailable'] };
  const stored = instance.fieldValues[sectionId] || {};
  const fields = (section.fields || []).filter((field) => field.required && fieldApplies(field, sectionId, instance.fieldValues) && (stored[field.name] == null || String(stored[field.name]).trim() === '')).map((field) => field.label);
  return { sectionName: section.name || section.title || sectionId, fields };
}
...
function expandRegisterTemplate(template, instance) {
  return String(template).replace(/\{\{([^}]+)\}\}/g, (match, path) => {
    ...
    if (path.startsWith('creationValues.')) {
      ...
      const value = instance.fieldValues?.[instance.workflowDefinition?.creationSection]?.[key]
        ?? instance.creationValues?.[key];
    ...
    const [sectionId, ...fieldParts] = path.split('.');
    if (fieldParts.length && instance.fieldValues?.[sectionId]) {
      const value = instance.fieldValues[sectionId][fieldParts.join('.')];
      return value == null ? '' : String(value);
    }
    return match;
  });
...
export function createInstance(workflow, input, actor, existingIds) {
  ...
  const relationshipSection = template.sections.find((section) => section.fields?.some((field) => field.name === 'relationship_type'));
  if (relationshipSection) {
    const relationship = relationshipSection.fields.find((field) => field.name === 'relationship_type')?.derivedValue;
    if (relationship) instance.fieldValues[relationshipSection.id] = { relationship_type: relationship };
  }
...
export function saveFieldValues(instance, workflow, sectionId, payload, actor) {
  ...
  const validation = validateFieldValues(template, sectionId, payload, instance.fieldValues);
  if (validation.errors.length) throw new Error(validation.errors.join(' '));
  instance.fieldValues[sectionId] = { ...(instance.fieldValues[sectionId] || {}), ...validation.values };
  const selectedPosition = validation.values.position_id && CANONICAL_POSITIONS.find((item) => item.id === validation.values.position_id);
  if (selectedPosition) {
    instance.fieldValues[sectionId].position_title = selectedPosition.title;
    instance.fieldValues[sectionId].position_source_document = selectedPosition.source_document;
    instance.fieldValues[sectionId].position_source_path = selectedPosition.source_path;
    instance.fieldValues[sectionId].position_source_hash = selectedPosition.source_hash;
  }
...
export function stageSatisfied(instance, workflow, index) {
  ...
  const errors = [];
  for (const rule of stage.blockingRules || []) {
    if (!conditionMatches(instance, rule.when)) errors.push(rule.message);
  }
  for (const sectionId of stage.sectionsRequired || []) {
    const missing = sectionMissingFields(instance, workflow, sectionId);
    if (missing.fields.length) errors.push(`Complete ${missing.sectionName}: ${missing.fields.join(', ')}.`);
  }
...
export function advanceStage(instance, workflow, actor) {
  ...
  let next = index + 1;
  instance.stageStates[index].status = 'complete';
  while (next < workflow.lifecycle.length && (workflow.lifecycle[next].satisfiedByIntake || !stageApplies(instance, workflow.lifecycle[next]))) {
    instance.stageStates[next].status = workflow.lifecycle[next].satisfiedByIntake ? 'complete' : 'not_applicable';
    next += 1;
  }
  if (next >= workflow.lifecycle.length) {
    instance.currentStageIndex = workflow.lifecycle.length;
    instance.status = 'completed';
    instance.completedAt = now;
    instance.updatedAt = now;
    instance.audit.unshift(event('instance_completed', actor, { completedStage: stage.id }));
...
export function saveDocumentFieldValues(instance, documentInstanceId, payload, actor) {
  ...
  const definition = ((instance.templateDefinition || getTemplate(instance.templateId))?.documents || []).find((item) => item.id === document.documentId);
  if (!definition) throw new Error(`Document definition ${document.documentId} is unavailable.`);
  const validationTemplate = { sections: [{ id: document.documentId, fields: definition.fields || [] }] };
  const validation = validateFieldValues(validationTemplate, document.documentId, payload, { [document.documentId]: document.fieldValues });
  if (validation.errors.length) throw new Error(validation.errors.join(' '));
...
export function finalizeDocument(instance, documentInstanceId, actor) {
  ...
  const definition = ((instance.templateDefinition || getTemplate(instance.templateId))?.documents || []).find((item) => item.id === document.documentId);
  if (!definition) throw new Error(`Document definition ${document.documentId} is unavailable.`);
  const missingFields = (definition.fields || []).filter((field) => field.required && fieldApplies(field, document.documentId, { [document.documentId]: document.fieldValues }) && (document.fieldValues[field.name] == null || String(document.fieldValues[field.name]).trim() === '')).map((field) => field.label);
  if (missingFields.length) throw new Error(`Document cannot be finalized until required fields are complete: ${missingFields.join(', ')}.`);
...
export function summarizeInstance(instance, workflow, template) {
  ...
  const completed = stageStates.filter((stage) => stage.status === 'complete' || stage.status === 'not_applicable').length;
  return {
    schema: instance.schema,
    schemaVersion: instance.schemaVersion || 1,
    id: instance.id,
