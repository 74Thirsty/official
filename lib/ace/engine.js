...
export function addEvidence(instance, workflow, actor, description, url = '') {
  if (instance.status !== 'active' && instance.status !== 'awaiting_approval') throw new Error('Completed workflows cannot be changed.');
  if (description && !String(description).trim().length) throw new Error('Evidence description is required.');
