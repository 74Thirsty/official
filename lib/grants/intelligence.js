 ...
 * @description Grant intelligence — search, filter, match
...
export async function syncOpportunities(opportunities, existingList, signal) {
  ...
  for (const hit of opportunities) {
    try {
      // searchGrants() already returns normalized records. Accept raw provider
      // hits too because this function is also used as the ingestion boundary.
      const fresh = hit?.provider === 'grants-gov' && hit.providerOpportunityId !== undefined
        ? hit
        : normalizeSearchHit(hit);
      const result = upsertOpportunity(existingList, fresh, mergeNormalized);
      if (result.isNew) {
        existingList.push(result.record);
        stats.newCount++;
      } else {
        const index = existingList.findIndex((item) => {
          const itemExternalId = item.externalId || buildExternalId(item);
          return itemExternalId === result.record.externalId;
        });
        if (index >= 0) existingList[index] = result.record;
        stats.updatedCount++;
      }
    } catch (err) {
      stats.errorCount++;
