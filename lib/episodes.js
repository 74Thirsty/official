...
export function validateEpisode(payload = {}, ctx = {}) {
  ...
  const streamKeyRef = clean(payload.streamKeyRef, 100);
  if (streamKeyRef && !KEY_REF_RE.test(streamKeyRef)) {
    errors.push('Stream key reference must be an env var name (letters, numbers, underscores).');
  }
