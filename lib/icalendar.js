/**
 * @file        icalendar.js
 * @description iCalendar event generation
...
function eventLines(event) {
  ...
  if (event.time) {
    const end = addOneHour(recurring ? event.date : (event.endDate || event.date), event.time);
    ...
    const finalDate = recurring ? event.date : (event.endDate || event.date);
    lines.push(`DTEND;VALUE=DATE:${compactDate(addDays(finalDate, 1))}`);
  }
