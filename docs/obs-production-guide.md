...
- The server maps each destination platform to its stream-key environment variable (facebook → `FB_STREAM_KEY`, youtube → `YOUTUBE_STREAM_KEY`, twitch → `TWITCH_STREAM_KEY`, owncast → `OWNCAST_STREAM_KEY`). Episodes do not store any stream-key reference; the key itself lives only in secrets:
...
- Stream keys (server-side only, never exposed): `FB_STREAM_KEY` (facebook), `YOUTUBE_STREAM_KEY`, `TWITCH_STREAM_KEY`, `OWNCAST_STREAM_KEY`.
