...
| Variable | Required | Value | Notes |
|---|---|---|---|
| `PAYPAL_DONATE_HOSTED_BUTTON_ID` | For online donations | PayPal Donate hosted button ID | Public/browser-safe identifier generated by PayPal. Configure suggested amounts, custom amounts, recurring giving, and any program choices in the PayPal button settings. |
| `PAYPAL_DONATE_ENVIRONMENT` | Optional | `sandbox` or `production` | Defaults to `production`; use `sandbox` only with a Sandbox hosted button ID. |
